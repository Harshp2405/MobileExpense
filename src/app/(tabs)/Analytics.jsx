...
export default function AnalyticsScreen() {
  ...
  // NEW: state for drill-down modal
  ...
  // NEW: given the currently loaded `expenses` + a category name, compute subcategory breakdown
  const getSubcategoryBreakdown = (categoryName) => {
    const filtered = expenses.filter(
      (e) => (e.category || "General") === categoryName,
    );
    const totals = {};
    filtered.forEach((e) => {
      const sub = e.subcategory || "Other";
      totals[sub] = (totals[sub] || 0) + Number(e.amount);
    });
  ...
  return (
      ...
      {loading ? (
        ...
        >
          ...
          <View className="flex-1 flex-col">
              ...
              <View className="bg-white dark:bg-zinc-800 p-6 rounded-3xl border border-gray-100 dark:border-zinc-700 shadow-sm">
                ...
                <View className="mt-5 space-y-3">
                  {categoryData.map((item, index) => (
                    <TouchableOpacity
                      key={index}
                      onPress={() => setDrilldownCategory(item.name)}
                    >
                      <View
                        key={index}
            ...
            <View className="px-6 pt-5">
              <View className="bg-white dark:bg-zinc-800 p-6 rounded-3xl border border-gray-100 dark:border-zinc-700 shadow-sm">
                <Text className="text-base font-black text-gray-900 dark:text-gray-100 mb-5"></Text>
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <TouchableOpacity
                  ...
                  className={`p-4 rounded-xl items-center mb-2 border ${
                    selectedMonth === index
    ...
    {/* NEW: simple drill-down modal */}
      <Modal
        visible={!!drilldownCategory}
        animationType="slide"
        transparent
        onRequestClose={() => setDrilldownCategory(null)}
      >
        <View className="flex-1 justify-end bg-black/40">
          <View className="bg-white dark:bg-zinc-800 rounded-t-3xl p-6 max-h-[70%]">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-xl font-bold text-gray-900 dark:text-gray-100">
                {drilldownCategory}
              </Text>
              <TouchableOpacity onPress={() => setDrilldownCategory(null)}>
            ...
            <FlatList
              data={
                drilldownCategory
                  ? getSubcategoryBreakdown(drilldownCategory)
                  : []
              }
              keyExtractor={(item) => item.name}
              renderItem={({ item }) => (
                <View className="flex-row justify-between py-2 border-b border-gray-100 dark:border-zinc-700">
