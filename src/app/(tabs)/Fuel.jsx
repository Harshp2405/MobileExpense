...
const Fuel = () => {
  ...
  const kmDriven = startKm && endKm ? parseFloat(endKm) - parseFloat(startKm) : 0;
  const currentAvg = kmDriven > 0 && litres ? (kmDriven / parseFloat(litres)).toFixed(1) : null;
  ...
  const handleSave = async () => {
    if (!startKm.trim() || !endKm.trim() || !litres.trim() || !pricePerLitre.trim()) {
      Alert.alert("Error", "Start km, End km, Litres, and Price are required");
      return;
    }
    ...
    try {
      ...
      setStartKm(""); setEndKm(""); setLitres(""); setPricePerLitre(""); setNote("");
    ...
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-zinc-900">
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const driven = item.startKm ? item.odometerKm - item.startKm : 0;
            const mileage = driven > 0 && item.litres ? driven / item.litres : null;
            return (
              <Swipeable
                ...
                <View className="flex-row justify-between items-start">
                  <View className="flex-1">
                    <Text className="text-base font-bold text-gray-900 dark:text-gray-100">
                      {item.startKm?.toFixed(0) || "?"} → {item.odometerKm.toFixed(0)} km
                    </Text>
                    <Text className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      {driven > 0 ? `${driven.toFixed(0)} km` : ""} • {item.litres}L × ₹{item.pricePerLitre}/L • {item.date}
                    </Text>
                    {item.note ? (
            ...
            <ScrollView>
              ...
              {currentAvg && (
                <View className="bg-green-50 dark:bg-green-900/20 p-3 rounded-xl mb-4 flex-row justify-between items-center">
                  <Text className="text-sm font-bold text-green-700 dark:text-green-400">Average Preview</Text>
                  <Text className="text-lg font-black text-green-600 dark:text-green-400">{currentAvg} km/L</Text>
                </View>
              )}
